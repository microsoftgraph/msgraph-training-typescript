...
1. Select **New registration**. Enter a name for your application, for example, `TypeScript App-Only Graph Tutorial`.
...
1. Open PowerShell and run the [RegisterAppForAppOnlyAuth.ps1](RegisterAppForAppOnlyAuth.ps1) file with the following command.
    ...
    ```powershell
    .\RegisterAppForAppOnlyAuth.ps1 -AppName "TypeScript App-Only Graph Tutorial" -GraphScopes "User.Read.All"
    ```
...
1. Rename [appSettings.example.ts](./graphapponlytutorial/appSettings.example.ts) to `appSettings.ts` and update the values according to the following table.
